...
# irrelevant. cmd, PowerShell, or an agent's tool call all run THIS file; it
# locates Git Bash, re-execs the real script with it, and fails with an
# install instruction if Git Bash is absent. WSL bash never enters the picture.
$ErrorActionPreference = 'Stop'

$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$sh = Join-Path $scriptDir 'curate-sessions.sh'
