...
| 19 | `validate` cannot validate a decisions file | **open** | found by the item-20 run — the third-party agent tried it and got `unknown option`. Three defects in one command, each reproduced (`logs/validate-defect.log`, `logs/validate-from.log`): (a) `validate FILE` is rejected as an unknown option although the help says "sanity-check a candidates/decisions TSV"; (b) `-o DIR` resolves only `DIR/candidates.tsv`, so it can never reach `decisions.tsv`; (c) the one working spelling, `--from FILE`, is documented only under `finalize options`, and its shape check then calls a well-formed decisions file **malformed** — the awk branch reads `want = ($1 == "decision") ? 10 : 7`, but a decisions TSV has 9 columns beginning with `agent`, so it expects 7 and reports `MALFORMED rows: 1`. It false-alarms on exactly the file it exists to check. Acceptance: `curate-sessions.sh validate DIR/decisions.tsv` exits 0 printing `shape ok (9 cols)` plus the decision tally, and `validate -o DIR` validates `decisions.tsv` when one is present |
| 20 | end-to-end: an unprimed third-party agent reaches the intended corpus | **done** | Windows codex 0.154.0, `codex exec --dangerously-bypass-approvals-and-sandbox`, prompt in `logs/codex-prompt.txt` (states no-review, no turn floor, non-coding; never names the target session). The installed copy was reinstalled to HEAD first and verified byte-identical, CR=0. Codex read `SKILL.md`, used the documented `.ps1` wrappers, ran `scan --agent both --min-lines 0`, screened into `screen.tsv`, then `finalize --yolo` and `delivery`: **34 candidates → 3 exported, including `翻译 桃花源记 为日文`**. PM verification independent of codex's report: archive magic `50 4b 03 04`, members are `keep/` ×3 + `manifest.json`, and every entry's sha256 matches **the original session file** byte-for-byte (`logs/codex-verify.log`); manifest records `mode=yolo`, `reviewed=false`, `approved_by=user-opt-out`. `writer: /c/Windows/System32/tar.exe -a -cf (bsdtar)` — the item-16 rung, exercised by a real caller |
