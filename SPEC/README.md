...
| 22 | a retired skill stays installed, telling users to install what no longer exists | **open** | Found by verifying the cutover on Windows: `skills add` copies and never removes, so `~/.agents/skills/trajectory-packs` survived the directory leaving the repository. Its driver is fail-closed (`rc=1`, writes nothing), but the remedy it prints — `npx skills add … --skill trajectory-packs --skill trajectory-funnel --skill agent-session-batch-export` — names two skills that no longer exist, and the installer answers `Selected 2 skills … Installed 2 skills ✓✓` while silently dropping the third. A user follows the instruction, is told it succeeded, re-runs, and hits the same error. This is the migration's own residue on partner machines, and no code in this repository can reach it. Requirement: retiring a published skill must leave the old copy able to say what replaced it. Acceptance: a documented uninstall/replacement note in the release notes and `README.md`, plus a check that no shipped script prints an install command naming a skill absent from `skills/` |
...
| 24 | two length controls, one user intent, no way to tell them apart | **open** | Found by the item-23 codex run, which closed with: "I set the session-length floor to zero, but that message-length rule still applied." A user who says "don't drop short sessions" is offered `--min-lines` (a **file line-count** floor, a CLI flag — codex correctly set it to `0`) while a second, unrelated floor `min_user_msg_chars` (a **message character** count, L5, inside each theme file) keeps killing sessions and is not reachable from the command line. The funnel table reports L5 truthfully (`longest later user msg 4 < 5`), so nothing is hidden — the defect is that the two controls are never distinguished for the caller, so satisfying the stated intent silently requires editing a theme, which the agent is explicitly forbidden to do. Not a threshold complaint: the theme values are policy and stay in the theme. Acceptance: `export-direction.sh --help` and SKILL.md each name both controls and say which one a "don't drop short sessions" request means, and a run whose L5 kills exist prints the theme key a caller would have to change rather than only the comparison |
