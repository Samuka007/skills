...
Maintainer and developer documentation. The user-facing front door — the
paste-and-export task brief — lives in the root [`README.md`](../README.md);
this file is about the repository's layout, tests and acceptance flow.
...
| [`session-export-nocode`](skills/session-export-nocode/SKILL.md) | Export the non-code direction: four themes (role-play, 写作, 策划, 报告分析), selected by the base skill's deterministic funnel rather than by an agent reading prose. Invoked by hand; unattended by default (`--confirm` restores the batch-level checkpoint). |
...
prose. The launcher is unattended by default (item 31); `--confirm` restores
the batch-level `[y/N]` checkpoint.
